// Endless Mode – po trafieniu runda blokowana; „Graj dalej” w overlayu i obok wyszukiwarki
...
$(document).ready(function () {
  ...
  function lockRound() {
    ...
    $("#pole_szukania")
      .prop("disabled", true)
      .attr("placeholder", "Runda zakończona — kliknij Graj dalej");
    $("#playAgainBtn").removeClass("d-none"); // pokaż natychmiastowy przycisk
    hideSuggestions();
  }
  ...
  function startNewRound() {
    ...
    $("#historiaBody, #historiaCards").empty();
    $("#pole_szukania")
      .val("")
      .prop("disabled", false)
      .attr("placeholder", "Wyszukaj samochód...");
    $("#playAgainBtn").addClass("d-none");
    tries = 0; updateTries();
    losujSamochod();
  }
  ...
  const samochodyZdjecia = {
    "Aventador": "modele/aventador.jpg","Huracan":"modele/huracan.jpg","Countach":"modele/countach.jpg","Diablo":"modele/diablo.jpg",
    "911 GT3 RS":"modele/911gt3rs.jpg","918 Spyder":"modele/918spyder.jpg","Carrera GT":"modele/carreragt.jpg",
    "Supra MK4":"modele/supramk4.jpg","Supra MK5":"modele/supramk5.jpg","GT-R":"modele/gtr.jpg","Skyline R34":"modele/skyline.jpg",
    "919 Hybrid Evo":"modele/919hybridevo.jpg","Gallardo":"modele/gallardo.jpg","Murcielago":"modele/murcielago.jpg",
    "Revuelto":"modele/revuelto.jpg","Urus":"modele/urus.jpg","F40":"modele/f40.jpg","F50":"modele/f50.jpg","LaFerrari":"modele/laferrari.jpg",
    "812":"modele/812.jpg","SF90":"modele/sf90.jpg","Enzo":"modele/enzo.jpg","Zonda":"modele/zonda.jpg","Huayra":"modele/huayra.jpg","Utopia":"modele/utopia.jpg",
    "M3 E30":"modele/m3e30.jpg","M3 E36":"modele/m3e36.jpg","M3 E46":"modele/m3e46.jpg","M3 E90/E92/E93":"modele/m3e92.jpg","M3 F80":"modele/m3f80.jpg","M3 G80":"modele/m3g80.jpg",
    "M4 F82":"modele/m4f82.jpg","M4 G82":"modele/m4g82.jpg","M5 E28":"modele/m5e28.jpg","M5 E34":"modele/m5e34.jpg","M5 E39":"modele/m5e39.jpg","M5 E60":"modele/m5e60.jpg",
    "M5 F10":"modele/m5f10.jpg","M5 F90":"modele/m5f90.jpg","AMG ONE":"modele/amgone.jpg","AMG GT Black Series":"modele/amggtblackseries.jpg",
    "R8":"modele/r8.jpg","RX-7":"modele/rx7.jpg","MX-5":"modele/mx5.jpg","EB110":"modele/eb110.jpg","Veyron":"modele/veyron.jpg","Chiron":"modele/chiron.jpg",
    "Tourbillon":"modele/tourbillon.jpg","Bolide":"modele/bolide.jpg","Jesko":"modele/jesko.jpg","One:1":"modele/one1.jpg","Agera":"modele/agera.jpg","F1":"modele/f1.jpg",
    "Senna":"modele/senna.jpg","720S":"modele/720s.jpg","P1":"modele/p1.jpg"
  };
  ...
  const renderWinOverlay = (imgSrc, marka, model) => {
    const html = `
      ...
      <div class="win-card">
        ...
        <div class="win-body">
          <div class="win-title mb-2">Brawo! Trafiłeś: <strong>${marka} ${model}</strong>.</div>
  ...
  // Zamykanie overlayu — runda już zablokowana, przycisk przy inputcie pozostaje
  $(document).on('click', '#winClose, #zamknijOverlay', function () {
    $("#win").removeClass("show").empty();
  });

  // „Graj dalej” – z overlayu LUB z przycisku przy wyszukiwarce
  $(document).on('click', '#grajDalej, #playAgainBtn', function () {
    startNewRound();
  });
  ...
  const showSuggestions = (items) => {
    if (!Array.isArray(items) || !items.length) {
      $("#sugestie").removeClass("d-none").html('<div class="list-group-item text-muted">Brak wyników…</div>');
      return;
    }
    const html = items.map(s => {
      const imgPath = samochodyZdjecia[s.model];
      const imgHtml = imgPath ? `<img class="car-thumb rounded" src="${imgPath}" alt="Zdjęcie ${s.model}">` : "";
      return `
  ...
  $("#pole_szukania").on("click", function () {
    ...
    $.ajax({ url: "all.php", method: "POST" })
      .done((data) => { let l=[]; try{l=JSON.parse(data)}catch{}; showSuggestions(l); })
      .fail(() => $("#sugestie").removeClass("d-none").html('<div class="list-group-item text-danger">Błąd ładowania listy.</div>'));
  });
  ...
  $("#pole_szukania").on("keyup", function () {
    ...
    $.ajax({ url: "search.php", method: "POST", data: { input: q } })
      .done((data) => { let l=[]; try{l=JSON.parse(data)}catch{}; if(l.length) showSuggestions(l); else $("#sugestie").removeClass("d-none").html('<div class="list-group-item text-muted">Brak wyników…</div>'); })
      .fail(() => $("#sugestie").removeClass("d-none").html('<div class="list-group-item text-danger">Błąd wyszukiwania.</div>'));
  });
  ...
  $(document).on('click', '.suggestion', function () {
    ...
    $("#pole_szukania").val(""); hideSuggestions();
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (wylosowanySamochod.model === wybranySamochod.model) {
          ...
          lockRound(); // blokada + pokaż przycisk obok pola
        } else {
          tries += 1; updateTries();
          ...
          const splitNap = (str) => { const s = String(str||'').replace(/\s+/g,''); const parts = s ? s.split(',') : []; return [parts[0]||null, parts[1]||null]; };
          const [pNapWyl1, pNapWyl2] = splitNap(wylosowanySamochod.napedy);
          const [pNapWybr1, pNapWybr2] = splitNap(wybranySamochod.napedy);
          const napedState = (()=>{
            if ((pNapWyl1 === pNapWybr1 && pNapWyl2 === pNapWybr2) || (pNapWyl1 === pNapWybr2 && pNapWyl2 === pNapWybr1)) return 'ok';
            if ([pNapWybr1,pNapWybr2].some(x => x && (x===pNapWyl1 || x===pNapWyl2))) return 'partial';
            return 'bad';
          })();

          const splitNad = (str) => { const s = String(str||'').replace(/\s+/g,''); const parts = s ? s.split(',') : []; return [parts[0]||null, parts[1]||null, parts[2]||null]; };
          ...
          const splitSkrz = (str) => { const s = String(str||'').replace(/\s+/g,''); const parts = s ? s.split(',') : []; return [parts[0]||null, parts[1]||null]; };
